...
let myPlaylists = ["romantic songs" , "motivational" , "party songs" ];
...
let likedSongs = [
  ...
  {songname: "Baarish Ka Asar - Twin Strings", filePath: "songs/10BaarishKaAsarmp3", coverPath: "cover/10.jpg", id:10}
]
//audioElement.play();

// Creating tabcontent/all songs of a playlist div
let container = document.getElementsByClassName("container");
myPlaylists.forEach((element, i)=>{
    let divl = document.createElement('div');
    divl.id = element;
    divl.classList.add("tabcontent");
    divl.innerHTML = `<h3 class="selectedList">${i+1}. ${element}</h3>`;
    container[0].appendChild(divl);

    // Populating songs
    likedSongs.forEach((element, i)=>{
        let newDiv = document.createElement("div");
        divl.appendChild(newDiv);
        //console.log(divl);
...
// Creating tablinks
let playlistTab = document.getElementsByClassName('tab');
myPlaylists.forEach((element , i)=>{
  //console.log(element);
  let tablinks = document.createElement('button');
  tablinks.classList.add("tablinks");
  tablinks.setAttribute("onclick" , `openPlaylist(event, "${myPlaylists[i]}")`);
  tablinks.innerText = i+1 + ". " +myPlaylists[i];
  playlistTab[0].appendChild(tablinks);
})
